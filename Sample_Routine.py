# -*- coding: utf-8 -*-
"""
Created on Thu Aug  6 15:07:32 2026

@author: thomasnguyen
"""
import os, sys, time

### Point script to correct directory to ensure dependencies are correctly loaded ###
try:
    script_dir = os.path.abspath(os.path.join(
        os.path.dirname((os.path.abspath(__file__)))))

except NameError:
    # __file__ is not defined (e.g., in an interactive environment)
    # Use sys.argv[0] to get the script's name and then extract 1 directory above current one
    script_dir = os.path.abspath(os.path.join(
        os.path.abspath(sys.argv[0])))

# if script_dir not in sys.path:
#     sys.path.append(script_dir)   
if os.getcwd() != script_dir: 
    script_dir = os.path.join(script_dir)
    os.chdir(script_dir) 
    
import numpy as np
import random
import pyvista as pv
import pyvista
from svv.domain.domain import Domain
from svv.forest.forest import Forest
from svv.tree.data.data import TreeParameters
from metric_calcs.calc_vessel_angles import calc_vessel_angles
from metric_calcs.calc_vessel_conn_tortuosity import calc_tortuosity
import pandas as pd
#%% Set up output directory for files to be saved in

output_dir = 'C:/Users/thomasnguyen/OneDrive - University of California, San Diego Health/00_Projects/01a_SVVascularize/svVascularize/'
full_output_dir = os.path.join(output_dir,'Simulation_Results/New_Angle_Test/')
if not os.path.exists(full_output_dir):
    while True:
        try:
            os.makedirs(full_output_dir)
            break
        except OSError:
            continue


#%% Plot function


def plot_forest_annotate(forest,filename,export_type):
    if export_type != 'save':
        plotter = pyvista.Plotter(notebook = False)
    elif export_type == 'save':
        plotter = pyvista.Plotter(notebook = False,off_screen=True,window_size=[1280, 1280])
    colors = ['red','blue']
    count = 0
    def _add_cylinder(p0, p1, radius, color, opacity=1.0):
        vec = p1 - p0
        length = np.linalg.norm(vec)
        if length <= 0:
            return
        direction = vec / length
        center = (p0 + p1) / 2
        cyl = pyvista.Cylinder(center=center, direction=direction, radius=radius, height=length)
        plotter.add_mesh(cyl, color=color, opacity=opacity)

    has_connections = getattr(forest, "connections", None) is not None and \
        getattr(forest.connections, "tree_connections", None)
    
    # for j in range(0,len(forest.networks[0])):
    #     for k in range(0,forest.networks[0][j].data.shape[0]):
    #         plotter.add_points(forest.networks[0][j].data[k,0:3], render_points_as_spheres=True, point_size=15,color = colors[j])
    #         plotter.add_points(forest.networks[0][j].data[k,3:6], render_points_as_spheres=True, point_size=15,color = colors[j])


    if has_connections:
        # Draw connected trees and connection vessels
        for net_idx, tree_conn in enumerate(forest.connections.tree_connections):
            for tree in tree_conn.connected_network:
                color = colors[count % len(colors)]
                for i in range(tree.data.shape[0]):
                    p0 = tree.data[i, 0:3]
                    p1 = tree.data[i, 3:6]
                    radius = tree.data.get('radius', i)
                    _add_cylinder(p0, p1, radius, color)
                count += 1

            # Connection vessels (between trees in this network)
            for tree_idx, vessel_list in enumerate(tree_conn.vessels):
                color = colors[tree_idx % len(colors)]
                for vessel in vessel_list:
                    for seg in vessel:
                        p0 = seg[0:3]
                        p1 = seg[3:6]
                        radius = seg[6]
                        _add_cylinder(p0, p1, radius, color)
    else:
        # Fall back to original visualization without connections
        for network in forest.networks:
            for tree in network:
                for i in range(tree.data.shape[0]):
                    center = (tree.data[i, 0:3] + tree.data[i, 3:6]) / 2
                    direction = tree.data.get('w_basis', i)
                    radius = tree.data.get('radius', i)
                    length = tree.data.get('length', i)
                    vessel = pyvista.Cylinder(center=center, direction=direction, radius=radius, height=length)
                    plotter.add_mesh(vessel, color=colors[count % len(colors)])
                count += 1
    plotter.show_axes()
    plotter.show_grid()
    plotter.add_mesh(forest.domain.boundary, color='grey', opacity=0.25)
    plotter.view_isometric()
    if export_type == 'display':
        plotter.show()
    elif export_type == 'save':
        plotter.screenshot(os.path.join(full_output_dir,'{}.png'.format(filename)))
#%%

forest_start_time = time.perf_counter()
# rand_seed = random.randrange(0,1e6) + os.getpid() + int(time.time()/1e6)
rand_seed =  3893922
rng = np.random.default_rng(rand_seed)
# 1. Lock the global Python random module & NumPy's Global state
random.seed(rand_seed)       # Python's built-in random
np.random.seed(rand_seed)    # NumPy's global state



# Creating the Tissue Domain
cube_mesh = pv.Cube(x_length = 2,y_length = 2,z_length = 2)
cube = Domain(cube_mesh)

# Enforce same rng seed on domain
cube.set_random_seed(rand_seed)
cube.set_random_generator()


cube.create()
cube.solve()
cube.build()

# Creating the Vascular Forest Object

# 1. Define the 3 starting locations (Inlet 1, Inlet 2, Outlet 1)
# These should be points on the surface or just inside your 'cube' domain
pt_artery = np.array([1, 1, -1]) 
pt_vein = np.array([-1, -1, 1]) 
# pt_artery = np.array([0, 1.5, 0]) 
# pt_vein = np.array([0, -1.5, 0]) 


# 2. Specify direction of growth of arteries/veins
dir_artery = np.array([-1,-1,1])
dir_vein = np.array([1,1,-1])

# Parameters to control vessel size
# Tree with custom parameters
artery_params = TreeParameters()
artery_params.terminal_pressure = 100  # mmHg equivalent in CGS
artery_params.root_flow = 70  # cm^3/s equivalent in CGS

vein_params = TreeParameters()
vein_params.terminal_pressure = 100  # mmHg equivalent in CGS
vein_params.root_flow = 70  # cm^3/s equivalent in CGS

network_params = [artery_params,vein_params]

# 3. Feed in initial parameters to generate forest
n_networks = 1
n_trees_per_network = [2]
forest = Forest(
    domain =  cube,
    n_networks= n_networks, 
    n_trees_per_network= n_trees_per_network,
    physical_clearance = 0.05,
    tree_parameters = network_params
        )
forest.set_domain(cube)

# 4. Append all starting points & directions into 1x3 array
all_start_points = [[pt_artery,pt_vein]]
all_directions = [[dir_artery,dir_vein]]

# 5. Set roots; note that volume_fraction controls root size
forest.set_roots(
    start_points=all_start_points,
    directions= all_directions,
    volume_fraction = 0.10,
    )

# 6. Add specified number of terminal vessels (2*N+1) for total vessels
# threshold: minimum distance (fraction of volume length) to search for terminal vessel length
# volume_threshold: maximum distance (fraction of volume length) to search for terminal vessel length
# n_points: how many points to randomly generate in space for candidate terminal vessel
# n_closest_vessels: of the number of n_points, choose the m-amount of closest
# angle_constraint: "new" means fixed angle constraint for terminal vessel generation; 
# "old" means error fixed angle constraint for terminal vessel generation'
# None means no angle constraint

forest.add(10,
           threshold = 0.15,
           volume_threshold = 0.8,
           n_points = 1000,
           n_closest_vessels = 100,
           angle_constraint = "new",
           )


# 7. Connect vessels between trees together
# assign_type: "old" means connected vessels chosen based on geodesic distance,
# "new" means connected vessels chosen based on geodesic distance and directionality
# lambda_1: penalty factor for geodesic distance on new assignment algorithm
# lambda_2: penalty factor for directionality distance on new assignment algorithm


lambda_1 = 1
lambda_2 = 1
forest.connect(assign_type = 'new',
               lambda_1 = lambda_1,
               lambda_2 = lambda_2)
forest_end_time = time.perf_counter()
forest_end_time = time.perf_counter()
print("Time to generate and connect forests is: {:.3f} seconds".format(forest_end_time - forest_start_time))


#%% Calculations 


# Calculate terminal vessel angles
t1_map,t1_data = forest.networks[0][0].vessel_map,forest.networks[0][0].data
t2_map,t2_data = forest.networks[0][1].vessel_map,forest.networks[0][1].data
t1_angle_data = calc_vessel_angles(t1_map,t1_data)
t1_angle_data['Tree'] = 1
t2_angle_data = calc_vessel_angles(t2_map,t2_data)
t2_angle_data['Tree'] = 2

all_tree_angles = pd.concat([t1_angle_data,t2_angle_data])
all_tree_angles['Random Seed'] = rand_seed



# Calculate connection tortuosity
vessel_assignments = forest.connections.tree_connections[0].assignments
t1_conn_vessels = forest.connections.tree_connections[0].vessels[0]
t2_conn_vessels = forest.connections.tree_connections[0].vessels[1]

tortuosity_df = calc_tortuosity(vessel_assignments,t1_conn_vessels,t2_conn_vessels)
tortuosity_df['Random Seed'] = rand_seed

# Save dataframes
all_tree_angles.to_csv(os.path.join(full_output_dir,'all_tree_angles.csv'))
tortuosity_df.to_csv(os.path.join(full_output_dir,'tortuosity_cals.csv'))

#%% Plot Network

plot_forest_annotate(forest,filename = 'cubic_test',export_type = 'save')

#%% Build meshes

from svv.simulation.simulation import Simulation

sim_start_time = time.perf_counter()

sim = Simulation(forest,name = 'Simulation_Results/Chloe_Structures/T_2/',directory = output_dir)

cap_resolution = 10
# Build All Surface/Volume Meshes for 3D CFD Simulation - Return network mesh
sim.build_meshes(boundary_layer = False,cap_resolution = cap_resolution)
sim_end_time = time.perf_counter()
print("Time to generate meshes is: {0:.3f} seconds for a cap resolution of {1:2d}.".format((sim_end_time - sim_start_time),cap_resolution))

#%% Save Meshes

if hasattr(sim, 'fluid_domain_surface_meshes') and sim.fluid_domain_surface_meshes:
    for i, mesh in enumerate(sim.fluid_domain_surface_meshes):
        filename = os.path.join(full_output_dir, f"surface_mesh_{i}.stl")
        mesh.save(filename)
        print(f"Saved: {filename}")

# Save volume meshes (.vtk)
if hasattr(sim, 'fluid_domain_volume_meshes') and sim.fluid_domain_volume_meshes:
    for i, mesh in enumerate(sim.fluid_domain_volume_meshes):
        filename = os.path.join(full_output_dir, f"volume_mesh_{i}.vtk")  # or .vtu
        mesh.save(filename)
        print(f"Saved: {filename}")

# Save forest object for easy reading next time
forest.save(os.path.join(full_output_dir,"vascular_network.forest"))